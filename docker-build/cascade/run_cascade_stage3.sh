#!/usr/bin/env bash
# Stage 3: Code RL + OPD
# Data: LiveCodeBench (release_v6)
# Reward: SandboxFusion code execution (pass/fail) + format reward (0.1)
# Resumes from: Stage 2 checkpoint
# Checkpoint: stage3_code/
#
# Prerequisites:
#   bash scripts/deploy_sandbox.sh   # Deploy SandboxFusion on port 8080
set -xeo pipefail
...
EXP_NAME="haidass_cascade_code_rl_opd"

DATA_PATH=${DATA_PATH:-$HOME/data/cascade}
REWARD_FN_PATH=${REWARD_FN_PATH:-/root/cascade/my_rewards.py}
CKPT_ROOT=${CKPT_ROOT:-/tmp/verl/checkpoints}

TRAIN_FILES="$DATA_PATH/code_train.parquet"
TEST_FILES="$DATA_PATH/code_test.parquet"

# Auto-detect latest Stage 2 checkpoint
if [ -z "${PREV_STAGE_CKPT:-}" ]; then
    PREV_STAGE_CKPT=$(ls -td "$CKPT_ROOT"/verl_cascade_stage2_science/*/global_step_* 2>/dev/null | head -1 || true)
fi
...
REWARD=(
    ...
    custom_reward_function.reward_kwargs.sandbox_fusion_url="$SANDBOX_URL"
    custom_reward_function.reward_kwargs.sandbox_timeout=10
    reward.num_workers=4
)

RESUME=()
if [ -n "${PREV_STAGE_CKPT:-}" ]; then
    echo "Resuming from Stage 2 checkpoint: $PREV_STAGE_CKPT"
    RESUME=(
        trainer.resume_mode=force
        trainer.resume_from_path="$PREV_STAGE_CKPT"
    )
else
    echo "WARNING: No Stage 2 checkpoint found. Training from scratch."
...
if ! curl -s "${SANDBOX_URL%/run_code}/docs" > /dev/null 2>&1; then
    echo "ERROR: SandboxFusion not responding at $SANDBOX_URL"
    echo "Deploy it first: bash scripts/deploy_sandbox.sh"
    exit 1
fi
echo "SandboxFusion OK: $SANDBOX_URL"
