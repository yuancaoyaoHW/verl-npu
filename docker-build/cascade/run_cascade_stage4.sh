...
EXP_NAME="haidass_cascade_tool_rl_opd"
...
TRAIN_FILES="$DATA_PATH/tool_train.parquet"
TEST_FILES="$DATA_PATH/tool_test.parquet"

# Auto-detect latest Stage 3 checkpoint
if [ -z "${PREV_STAGE_CKPT:-}" ]; then
    PREV_STAGE_CKPT=$(ls -td "$CKPT_ROOT"/verl_cascade_stage3_code/*/global_step_* 2>/dev/null | head -1 || true)
fi
...
if [ -n "${PREV_STAGE_CKPT:-}" ]; then
    echo "Resuming from Stage 3 checkpoint: $PREV_STAGE_CKPT"
    RESUME=(
        trainer.resume_mode=force
        trainer.resume_from_path="$PREV_STAGE_CKPT"
    )
else
    echo "WARNING: No Stage 3 checkpoint found. Training from scratch."
