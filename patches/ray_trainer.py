...
# --- [verl-patch] Wave 3: Next-batch prefetch for pipeline overlap ---
...
class RayPPOTrainer:
    ...
    def __init__(
        ...
        # [verl-patch] Wave 3: Prefetch state for next-batch pipeline overlap
        ...
        for epoch in range(current_epoch, self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                with marked_timer("step", timing_raw):
                    ...
                    with marked_timer("gen", timing_raw, color="red"):
                        ...
                        if self._next_gen_future is not None:
                            # Use prefetched result from previous step
                            combined_gen_output = self._next_gen_future.result()
                            self._next_gen_future = None
                        else:
                            # First step or prefetch not available
                    ...
                    if self.config.trainer.critic_warmup > self.global_steps:
                        ...
                        # [verl-patch] Wave 3: Next-batch prefetch placeholder
                        # Full async prefetch requires generate_sequences_async support.
                        # Currently a no-op placeholder; activate when vLLM supports
                        # generation with pending weight updates.
                        self._next_gen_future = None
                ...
                if is_last_step:
                    ...
                    # [verl-patch] Cleanup prefetch
                    if self._next_gen_future is not None:
                        self._next_gen_future.cancel()
                        self._next_gen_future = None
